...
export class SignInComponent implements OnInit {
 ...
 project={url:''}
  ...
  getProjects(){
    this._dashService.get(`/services/app/HomePanal/GetAllSigneInPanal?tenantId=1`).subscribe(
      (res=>{
        this.isLoaded=true
        //@ts-ignore
        if(res['success']){
...
images:any[]=[];
 ...
 uploadFile = (files: FileList | null ) => {
  ...
  addAbout(){
    this.isLoaded=false;
    this._dashService.post(`/HomePanal/CreateNewSigneIn`,this.formData).subscribe(
      (res=>{
        //@ts-ignore
        if(res['success']){
    this.isLoaded=true;
 ...
 imgUrl:string='https://villaonline.co/wwwroot/Uploads/panal/';
    ...
    this.isLoaded=false;
      ...
      this._dashService.delete(`/services/app/HomePanal/DeleteSigneIn?Id=${id}`).subscribe((response) => {
        //@ts-ignore
        if(response['success']){
    this.isLoaded=true;
